...
{
// if(bitstream_framenum>=Start_Frame) {
  if (picture_structure==FRAME_PICTURE && Second_Field)
  {
    /* recover from illegal number of field pictures */
    imp(2,"odd number of field pictures\n");
    Second_Field = 0;
  }
...
resync: /* if Fault_Flag: resynchronize to next next_start_code */
    ...
    if (MBA>=MBAmax)
    {
      ...
      imp(2,"Too many macroblocks in picture\n");
      return(-1);
    }
  ...
  int *pdct_type;
{
  ...
  {
    imp(2,"maroblock_modes(): unknown macroblock type\n");
    motion_type = -1;
  }
#endif
...
{
  if (Second_Field)
    imp(2,"last frame incomplete, not stored\n");
  else
...
int PMV[2][2][2];
{
  ...
  if (code<SLICE_START_CODE_MIN || code>SLICE_START_CODE_MAX)
  {
    ...
    imp(2,"start_of_slice(): Premature end of picture\n");
  ...
  if (base.scalable_mode==SC_DP)
  {
    ...
    {
      /* only slice headers are allowed in picture_data */
      imp(2,"DP: Premature end of picture\n");
      return(-1);    /* trigger: go to next picture */
    }
  ...
  *MBAinc = Get_macroblock_address_increment();
  ...
  if (Fault_Flag) 
  {
    imp(2,"start_of_slice(): MBAinc unsuccessful\n");
    return(0);   /* trigger: go to next slice */
  }
